...
var queryStr = url.substr(queryStartsAt, url.length - queryStartsAt);
...
var identifier = queryStr.substr(fileIdStartsAt + 1, queryStr.length - fileIdStartsAt);
...
var req = request("GET", serverName + "/metadownload?file=" + identifier);
req.onload = function () {
    ...
    if (req.status === 200) {
        ...
        document.getElementById("fileDescription").innerHTML = file.filename;
        document.getElementById("createdAt").innerHTML = file.createdAt;
        document.getElementById("repositoryFile").style.display = "block";

        if(file.isSecured === false) {
            document.getElementById("secureKey").style.display = "none";
        }
        ...
        document.getElementById("downloadButton").addEventListener("click", function() {
            ...
            if (file.isSecurede && key.length === 0) {
                ...
                file.accessKey = key;
                var downloadReq = requestJson("POST", serverName + "/file", file);
                ...
                downloadReq.onload = function() {
                    ...
                    if(downloadReq.status === 200) {
                        ...
                        var disposition = downloadReq.getResponseHeader('content-disposition');
                        var matches = /"([^"]*)"/.exec(disposition);
                        var filename = (matches != null && matches[1] ? matches[1] : file.filename);
